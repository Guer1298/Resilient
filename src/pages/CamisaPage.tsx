...
export const CamisaPage = () => {
		...
		return (
			product?.variants.reduce(
				(acc: Acc, variant: VariantProduct) => {
					const { color, color_name, talla } = variant;
					if (!acc[color]) {
						acc[color] = {
							name: color_name,
							tallas: [],
					...
					if (!acc[color].tallas.includes(talla)) {
						acc[color].tallas.push(talla);
					}
    ...
    // Actualizar el talla seleccionado cuando cambia el color
	useEffect(() => {
		if (selectedColor && colors[selectedColor] && !selectedTalla) {
			setSelectedTalla(colors[selectedColor].tallas[0]);
		}
	}, [selectedColor, colors, selectedTalla]);
	...
	}, [selectedColor, selectedTalla, product?.variants]);
    ...
    return (
        ...
        <>
            <div className="h-fit flex flex-col md:flex-row grap-16 mt8">
