...
export const getOrderById = async (orderId: number) => {
	...
	const { data: order, error } = await supabase
		.from('orders')
		.select(
			'*, addresses(*), customers(full_name, email), order_items(quantity, price, variants(color_name, talla, products(name, images)))'
		)
		.eq('customer_id', customerId)
		.eq('id', orderId)
		.single();
	...
	return {
		...
		orderItems: order.order_items.map((item: { quantity: any; price: any; variants: { color_name: any; talla: any; products: { name: any; images: any[]; }; }; }) => ({
			quantity: item.quantity,
			price: item.price,
			color_name: item.variants?.color_name,
			talla: item.variants?.talla,
